...
public class PoissonDiscSampler
{
    ...
    public IEnumerable<Vector2> Samples()
    {
        ...
        yield return AddSample(new Vector2((float)rand.NextDouble() * rect.width, (float)rand.NextDouble() * rect.height));

        while (activeSamples.Count > 0)
        {

            // Pick a random active sample
            int i = (int)(float)rand.NextDouble() * activeSamples.Count;
